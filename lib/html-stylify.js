...
const HtmlStylify = function(parser){
    ...
    this.process = function(input, options){
        ...
        _uniquePrefix = _options.uniquePrefix ? _options.uniquePrefix : 'hs-' + Date.now()
    ...
    function _processCssAST(){
        _cssAST.stylesheet.rules.forEach(rule => {
            if(rule.type === 'rule'){
                ...
                rule.rules[0].selectors = _processRules(rule.rules[0].selectors)
            }
        })
    }
    ...
    function _traverseDomNodes(DOMNodes){
        DOMNodes.forEach((e, ndx, nodes) => {
            ...
            if(_options.removeScripts){
                if(e.type === 'tag' && e.name === 'script'){
                    nodes.splice(ndx, 1)
                }
            }
            //replace html, head, body and remove meta, title, comments
            if(_options.normalizeHtml){
                if(e.type === 'tag' && (e.name === 'html' || e.name === 'body')){
                    _addClass(e, addClass)
                    _cssProcessedElements.delete(e.name)
                    e.name = _options.replaceElement
                }
                if(e.type === 'tag' && e.name === 'head'){
                    e.attribs.style = e.attribs.style ? (e.attribs.style + ' display: none;') : 'display: none;'
                    e.name = 'span'
                }
