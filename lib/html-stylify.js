...
const HtmlStylify = function(){
    ...
    function _parseCss(cssString){
        ...
        try{
            ...
            if(_cssAST.stylesheet.parsingErrors.length > 0) {
                //if attempt is a fail, it's better to remove the style node
    ...
    function _processCssAST(){
        _cssAST && _cssAST.stylesheet.rules.forEach(rule => {
            if(rule.type === 'rule'){
        ...
        if(element && !element.processed){
            ...
            if(element.type === 'style'){
                element.children.forEach(e => {
                    _parseCss(e.data)
                    e.data = _cssAST ? css.stringify(_cssAST) : null
                })
            }
            //remove scripts
